...
public:
  ...
  int GetPrimHashCd() const {return Val1.GetPrimHashCd()+Val2.GetPrimHashCd();}
  int GetSecHashCd() const {return Val1.GetSecHashCd()+Val2.GetSecHashCd();}
...
void TVec<TVal>::Minus(const TVec<TVal>& ValV, TVec<TVal>& DstValV) const {
  ...
  int ValN1=0; int ValN2=0;
  while (ValN1<Len() && ValN2<ValV.Len()) {
    while (ValN1<Len() && GetVal(ValN1)<ValV.GetVal(ValN2)) {
        DstValV.Add(GetVal(ValN1)); ValN1++;
    }
    if (ValN1<Len()) {
        const TVal& Val1 = GetVal(ValN1);
        while (ValN2<ValV.Len() && Val1>=ValV.GetVal(ValN2)) { ValN2++; }
...
public:
  ...
  TVal& Top(){Assert(0<ValV.Len()); return ValV[ValV.Len()-1];}
  void Push(){ValV.Add();}
