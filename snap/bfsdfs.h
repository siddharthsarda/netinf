...
template <class PGraph> int GetNodesAtHops(const PGraph& Graph, const int& StartNId, const int& Hops, TIntV& NIdV, const bool& Dir=false);
...
template <class PGraph> 
int GetNodesAtHops(const PGraph& Graph, const int& StartNId, const int& Hops, TIntV& NIdV, const bool& Dir) {
  TBreathFS<PGraph> BFS(Graph);
  BFS.DoBfs(StartNId, true, !Dir, -1, Hops);
  NIdV.Clr(false);
  for (int i = 0; i < BFS.NIdDistH.Len(); i++) {
    if (BFS.NIdDistH[i] == Hops) {
